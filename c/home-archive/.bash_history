ls
cd learn2prog/
emac README 
ls
emacs README 
ll
cd learn2prog/
ll
cd 00_hello/
ll
emacs README 
fg
cd ,,.
cd ../
fg
cd 00_hello/
ll
emacs README 
ll
cd learn2prog/
ll
emacs README 
cd 00_hello/
ll
emacs README 
fg
git add hello.txt 
git commit -m "Did assignment 0"
git push
fg
grade
git add .
grade
git add .
git commit -m "readme"
git push
grade
ll
git status
cd ../
ll
rm .#README 
git status
rm \#README# 
ll
grade
cd 00_hello/
grade
ll
cd learn2prog/01
cd learn2prog/01_apple/
clear
cat README 
ecma fruit.txt
emacs fruit.txt
git status
ll
fg
git status
cat fruit.txt
rm .#fruit.txt fruit.txt-
git status
rm fruit.txt-
rm -f fruit.txt-
git add .
git commit -m "feat: 2-fruit"
git push
grade
git pull
grade
cd learn2prog/02_code1/
cat README 
ll
cd learn2prog/02_code1/
cat README 
ll
ecmas code1.c 
vi code1.c 
./test.sh 
emacs code1.c 
./test.sh 
git status
git add .
git commit -m "learn2prog/02_code1"
git push
grade
cd ../
ll
rm 01_apple/#fruit.txt# 
grade
cd 02_code1/
grade
git pull
cd learn2prog/03_code2/
cat README 
emacs code2.c 
./test.sh 
fg
emacs code2.c 
./test.sh 
fg
emacs code2.c 
./test.sh 
emacs code2.c 
./test.sh 
emacs code2.c 
./test.sh 
emacs code2.c 
./test.sh 

git status
git add .
git commit -m "learn2prog/03_code2"
git push
grade
git pull
grade
e
ll
cat grade.txt 
./test.sh 
emacs code2.c 
./test.sh 
git add .
git commit -m "2"
git push
grade
git pull
ll
cd learn2prog/04_compile/
cat README 
ecmas 
emacs
fg
emacs hello.c
git status
gcc hello.c -o hello
./hello 
git staus
git status
git add .
git commit -m "hello"
git push
grade
cd 
ll
cd learn2prog/05_squares/
la
ecmas squares.c
emacs squares.c
fg
emacs squares
emacs
fg 
reset
cd learn2prog/05_squares/
la
emacs squares.c
./square 4 1  2 3
./squares 4 1  2 3
./squares 3 5 8 2
grade
la
emacs squares.c
fg
ll
cat squares
cat squares*
ll
cat ans_3_5_8_2.txt 
./squares 3 5 8 2
cd learn2prog/
cd 05_squares/
ll
emacs squares.c
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
./squares 4 1  2 3
./squares 3 5 8 2
ll
emacs squares.c
fg
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
ll
cd learn2prog/
cd 05_squares/
la
ecmas squares.c
emacs squares
emacs squares.c
fg
g
fg
emacs squares.c
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
fg
emacs squares.c
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
./squares 4 1  2 3
la
diff ./squares 3 5 8 2 ans_3_5_8_2.txt 
git status
git add squares.c
git 
cd learn2prog/05_squares/
ll
git add .
git commit -m "square"
git push
grade
git pull
la
cat grade.txt 
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
fg
emacs squares.c
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
fg
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
fg
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
git add .
git commit -m "square"
git push
garde
grade
git pull
ll
cat grade.txt 
./squares 11 7 3 1
./squares ./squares 1 0 1 5
./squares  1 0 1 5
ll
./squares 3 5 8 2 | diff ans_3_5_8_2.txt 
/squares 3 5 8 2
./squares 3 5 8 2
emacs squares
emacs squares.c
ps aux
fg
emacs squares.c
fg
gcc -o squares -Wall -Werror -std=gnu99 --pedantic squares.c squares_test.o
./squares 4 1  2 3
./squares 3 5 8 2
emacs squares.c
./squares 4 1  2 3
./squares 3 5 8 2
git add .
git commit -m "square"
git push
git pull
grade
ll
cd learn2prog/06_rect/
ll
cat rectangle
cat README 
cd learn2prog/06_rect/
cat README 
ll
cd learn2prog/
ll
cd 06_rect/
ll
vi rectangle.c
grade
git add .
git commit -m "06"
git push
git pull
grade
git pull
ll
cat  grade.txt 
;;
ll
cd learn2prog/
ll
cd 06_rect/
ll
cd learn2prog/06_rect/
ll
vi rectangle.c
git add .
git commit -m "06"
git push
git pull
grade
cd learn2prog/06_rect/
git pull
cat grade.txt 
ll
cd learn2prog/06
cd learn2prog/06_rect/
ll
make
gcc -o rectangle -pedantic -std=gnu99 -Wall -Werror rectangle.c
./rectangle 
cat grade.txt 
clear
ll
cd learn2prog/06_rect/
la
vi rectangle.c 
rm rectangle.c
vi rectangle.c
git add .
git commit -m "6"
git push
grade
git pull
cat grade.txt 
vi rectangle.c
git add .
git commit -m"a"
git push
grade
git pull
cat grade.txt 
cd learn2prog/06_rect/
ll
vi rectangle.c
cd learn2prog/
cd 06_rect/
vi rectangle.c
make
./rectangle 
diif rectangle_ans.txt ./rectangle
diff rectangle_ans.txt ./rectangle
git add .
git commit -m "6"
git push
git pull
grade
git pull
cat grade.txt 
la
rm rectangle.c
vi rectangle.c
git status
git add .
git commit -m "6"
git push
git pull
grade
cd learn2prog/06_rect/
ll
cd ../
cd 07_retirement/
ll
vi retirement.c
make
./retirement 
git add .
git commit -m "7
"
git push
grade
git pull
cat grade.txt 
vi retirement.c
git status .
git commit -m "7"
git push
grade
git add .
git commit -m "7"
git push
grade
cd learn2prog/08_testing/
ll
./isPrime-correct  3
./isPrime-correct 4
ll
./isPrime-broken1 0
./isPrime-broken1 1
./isPrime-broken1 -1
la
./isPrime-broken1 a
./isPrime-broken1 ?
./isPrime-correct ?
./isPrime-broken1 1 1
./isPrime-correct 1 1
./isPrime-broken1 10
./isPrime-broken1 -100
./isPrime-broken1 (1+2)
./isPrime-broken1 1+2
uname -a
ifconfig
cd learn2prog/08_testing/
la
./isPrime-correct 
./isPrime-broken1 
./isPrime-broken1 -1000000
./isPrime-broken1 13
./isPrime-broken1 12
./isPrime-broken1 -10
./isPrime-correct -10
./isPrime-broken2 -10
./isPrime-broken3 -10
./isPrime-broken4 -10
vi input.4
./isPrime-correct ha
./isPrime-broken1 ha
./isPrime-broken2 ha
./isPrime-broken3 ha
vi input.3
./isPrime-correct 0.01
./isPrime-broken1 0.01
./isPrime-broken2 0.01
./isPrime-correct 2,147,483,647
./isPrime-correct 2147483647
./isPrime-broken1 2147483647
./isPrime-broken2 2147483647
vi input.2
./isPrime-correct 0
./isPrime-broken1 0
./isPrime-correct 1.2
./isPrime-broken1 1/2
./isPrime-broken1 1.2
cd learn2prog/
la
cd 08_testing/
la
./isPrime-correct 1
./isPrime-broken1 1
vi input.1
git add .
git commit -m "8"
git push
grade
cat /usr/local/l2p/match5/correct-match5
u
clear
cd /usr/local/l2p/match5/correct-match5
cd /usr/local/l2p/match5
cat correct-match5 
cd ~
cd learn2prog/08_testing/
clear
/usr/local/l2p/match5/correct-match5 aaaaa bbbbb
/usr/local/l2p/match5/correct-match5 aaaaa bbbbc
/usr/local/l2p/match5/correct-match5 aaabb bbbbc
echo "aaaaa bbbbb" >> tests.txt
run_all
./run_all
cd ../
cd 09_testing2/
echo "aaaaa bbbbb" >> tests.txt
./run_all.sh 
cat tests.txt 
echo "aaaaa bbbbb" >> tests.txt
cat
cat tests.txt 
echo "aaaaa bbbb111b" >> tests.txt
echo "aa22aaa bbbb111b" >> tests.txt
echo "aaaaa bbbbb haha" >> tests.txt
./run_all.sh 
cat tests.txt 
echo "11" >> tests.txt
echo "" >> tests.txt
echo "? ?" >> tests.txt
run_all
./run_all.sh 
cat tests.txt 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
git add tests.txt 
git commit -m "9"
git push
grade
git add .
git commit -m "9"
rm ../08_testing/tests.txt
git add .
git commit -m "9"
git push
grade
cat grade.txt 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
git add .
git commit -m "9"
git push
grade
cd learn2prog/09
cd learn2prog/09_testing2/
cd ../
cd 10_gdb/
la
cat re
cat README 
cd learn2prog/10_gdb/
./game
make
game: game.c
gcc -o game -pedantic -std=gnu99 -Wall -Werror game.c
gdb
emacs game.c 
fg
cd learn2prog/10_gdb/
emacs game.c
gdb game.c
ecmas game
emacs game
fg
cd learn2prog/10_gdb/
la
emacs game
cd learn2prog/10_gdb/
ll
emacs game.c
gdb game
emacs game.c
fg
cd learn2prog/08_testing/
cd ../
cd 10_gdb/
la
emacs game.c
gdb game
./game 
vi input.txt
./game < input.txt
gdb game
cd learn2prog/10_gdb/
gdb game
vi input.txt 
./game < input.txt 
git status .
git add .
git commit -m " 10"
git push
grade
cd learn2prog/c2prj1_cards/
la
vi cards.c
rm cards.c
vi cards.c
git add .
git commit -m "c"
git push
grade
git pull
cat grade.txt 
la
cd learn2prog/c2prj1_cards/
la
rm cards.c
vi cards.c
git commit -m "c"
git add .
git commit -m "c"
git push
grade
git pull
cat grade.txt 
vi cards.
rm cards.
vi cards.c
git commit -m "c"
rm .cards..swp 
git add .
git commit -m "c"
git push
grade
git pull
grade
cat grade.txt 
vi cards.c
git add .
git commit -m "c"
git push
grade
git pull
cat grade.txt 
vi cards.c
la
cd learn2prog/c2prj1_cards/
la
vi cards.c
git add .
git commit -m "c"
git push
grade
make poker
la
./poker 
vi input.txt
./poker input.txt 
vi cards.c
make clean
make poker
./poker input.txt 
gdb ./poker 
sudo gdb ./poker 
n
gdb ./poker 
gdb ./poker input.txt
./poker input.txt 
la
ulimit -c
vi make
vi Makefile 
make poker 
./poker input.txt 
la
ll
ulimit -c
la 
vi Makefile 
make poker 
./poker input.
./poker input.txt 
la
vi Makefile 
ulimit -c unlimited
cd learn2prog/c2prj1_cards/
la
make poker 
./poker input.txt 
la
find . -name "core*"
ulimit -c unlimited
gdb ./poker input.txt
la
rm cards.c
vi cards.c
make poker 
./poker input.txt 
git add .
git commit -m "c"
git push
grade
git pull
cat grade.txt 
rm cards.c
vi cards.c
git add .
git commit -m "c
"
git push
garde
grade
git pull
cat grade.txt 
vi cards.c
git add .
git commit -m "C"
git push
grade
la
cd learn2prog/c2prj2_testing/
./test-eval example.txt
./run_all.sh 
la
vi example.txt 
cd learn2prog/c2prj2_testing/
la
cat example.txt 
./run_all.sh 
vi example.txt 
cd learn2prog/c2prj2_testing/
vi example.txt 
./run_all.sh 
vi example.txt 
cd learn2prog/c2prj2_testing/
clear
ecmas example.txt 
emacs example.txt 
fg
./run_all.sh 
fg
cat run_all.sh 
cat README 
fg
./run_all.sh 
fh
fg
cat example.txt 
fg
vi example.txt 
./run_all.sh 
cat example.txt 
vi example.txt 
la
./test-eval example.txt
vi example.txt 
./test-eval example.txt
./run_all.sh 
./test-eval example.txt
vi example.txt 
./run_all.sh 
cd learn2prog/c2prj2_testing/
vi example.txt 
./run_all.sh 
vi example.txt 
./run_all.sh 
la
./test-evale example.txt 
test-evale example.txt 
test-eval example.txt 
./test-eval example.txt 
vi example.txt 
./run_all.sh 
cat run_all.sh 
./run_all.sh example.txt 
cat 
cat README 
vi test
cat example.txt 
vi tests.txt
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi example.txt 
vi tests.txt 
./run_all.sh 
la
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
cat README 
vi tests.txt 
cd learn2prog/c2prj2_testing/
vi tests.txt 
./run_all.sh 
vi tests.txt
la
cd learn2prog/c2prj2_testing/
la
./test-eval tests.txt 
vi tests.txt 
cd learn2prog/c2prj2_testing/
la
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
./tests
vi tests.txt
./run_all.sh 
vi tests.txt
./run_all.sh 
cat tests.txt
vi tests.txt
./run_all.sh 
cat tests.txt
la
./test-eval tests.txt
vi tests.txt
cat tests.txt
vi tests.txt
./run_all.sh 
la
./test-eval tests.txt
vi tests.txt
la
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
la
./test-eval tests.txt 
./run_all.sh 
vi tests.txt
la
./test-eval la
./test-eval tests.txt 
./run_all.sh 
la
ll
./run_all.sh 
la
./test-eval tests.txt
vi tests.txt
./run_all.sh 
./r.ala
la
cd learn2prog/c2prj2_testing/
la
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
cd learn2prog/c2prj2_testing/
vi tests.txt
./run_all.sh 
cd learn2prog/c2prj2_testing/
vi tests.txt 
./run_all.sh 
git add .
git commit -m "c"
git push
grade
git status
rm ../c2prj1_cards/.cards.c.swn
git status
git rm ../c2prj1_cards/.cards.c.sw
git stastu
git status
git add .
git status
cd ../
git add .
git commit -m "c2"
cd c2prj2_testing/
git push
grade
git pull
cat grade.txt 
vi tests.txt 
./run_all.sh 
la
./test-eval tests.txt 
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
la
vi tests.txt 
cd learn2prog/c2prj2_testing/
la
vi tests.txt 
cd learn2prog/c2prj2_testing/
vi tests.txt 
la
cd learn2prog/c2prj2_testing/
la
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
cd learn2prog/c2prj2_testing/
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
cleaer
clear
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
git add .
git commit -m"c"
git push
grade
cat grade.txt 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
vi test.
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
la
cd learn2prog/c2prj2_testing/
la
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
la
./test-eval tests.txt 
cd learn2prog/c2prj2_testing/
la
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
cd learn2prog/c2prj2_testing/
./run_all.sh 
./test-eval testa
./test-eval tests.txt 
vi tests.txt 
la
cd learn2prog/c2prj2_testing/
vi tests.txt 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
git add .
git commit -m "c"
git push
grade
git pull
cat grade.txt 
cd learn2prog/c2prj2_testing/
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
la
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
./test-eval tests.txt 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
vi tests.txt 
./test-eval tests.txt 
./run_all.sh 
cat README 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
cat README 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
vi tests.txt 
./run_all.sh 
git push
git add .
git commit -m "c"
git push
grade
cd learn2prog/
la
git add .
git commit -m "Initial commit for Course 3"
git push
git pull
la
cd 11_read_ptr1/
cat r
cat README 
la
gcc test.c 
la
./a.out 
./a.out > answer.txt
la
vi Makefile
make
./test 
la
cat answer.txt 
git add .
git commit -m "11"
git push
grade
cdl
cd learn2prog/
la
cd 12_read_ptr2/
la
cp ../11_read_ptr1/Makefile .
la
vi Makefile 
make
la
./test 
./test > answer.txt
la
git add .
git commit -m "12"
git push
grade
cd learn2prog/13_read_arr1/
vi answer.txt
git add .
git commit -m "13"
git push
grade
git pull
cat grade.txt 
vi answer.txt 
git status
git add .
git commit  -m "13"
git push
grade
git pull
cat grade.txt 
git status
la
vi test.c 
vi Makefile
git add .
git status
git commit -m "13"
git push
grade
cd learn2prog/14_array_max/
la
vi arrayMax.c 
git add .
git commit -m "14"
git push
grade
git pull
git push
git pull
la
git status
cd ../
git add .
git commit -m "14"
git push
cd 14_array_max/
grade
cd learn2prog/15_tests_subseq/
./run_all.sh 
cd /usr/local/l2p/subseq
la
cd ~/learn2prog/15_tests_subseq/
la
"#include <stdio.h>
#include <stdlib.h>

int main(void) {
    int array1[] = { 77, 33, 19, 99, 42, 6, 27, 4};
    int array2[] = { -3, -42, -99, -1000, -999, -88, -77};
    int array3[] = { 425, 59, -3, 77, 0, 36};

    size_t maxSeq (array1, 8);
    size_t maxSeq (array2, 7);
    size_t maxSeq (array3, 6);
    size_t maxSeq (NULL, 0);
    size_t maxSeq (array1, 0);

    return EXIT_SUCCESS;
}
">> test-subseq.c
vi test-subseq.c 
./run_all.sh 
vi test-subseq.c 
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
git add .
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
cd learn2prog/15_tests_subseq/
la
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
cd learn2prog/15_tests_subseq/
la
vi test-subseq.c
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
la
rm test-subseq
la
rm .test-subseq.c.swn
rm .test-subseq.c.swp
rm .test-subseq.c.swo
la
./run_all.sh 
;a
la
./test-subseq 
la
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
./test-subseq 
vi test-subseq.c
./run_all.sh 
./test-subseq 
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
/usr/local/l2p/subseq 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
cd learn2prog/15_tests_subseq/
la
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq
vi test-subseq.c
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
rm test-subseq.c
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
vi test-subseq.c
./run_all.sh 
git status
git add .
git commit -m "15"
git push
grade
cd learn2prog/16_subseq/
la
vi maxSeq.c
git add .
git commit -m "16"
git push
grade
cd ../
git add .
git commit -m"16"
cd 16_subseq/
git push
grade
git pull
cat grade.txt 
vi maxSeq.c 
git add .
git commit -m "16"
git push
grade
git pull
cat grade.txt 
la
vi maxSeq.c 
git status
git add .
git commit -m "16"
git push
grade
git pull
cat grade.txt 
git pull 
cat grade.txt 
vi maxSeq.c 
git add .
git commit -m "16"
git push
grade
git pull
cat grade.txt 
vi maxSeq.c 
git add .
git commit -m "16"
git push
grade
cd learn2prog/17_read_arr2/
la
vi answer.txt
vi Makefile
git status
git status .
git add .
git commit -m "17"
git push
grade
cd learn2prog/18_reverse_str/
la
vi reverse.c
rm reverse.c
vi reverse.c
git add .
git commit -m "18"
git push
grade
git pull
cat grade.txt 
vi reverse.c
git add .
git commit -m "18"
git push
grade
git pull
cat grade.txt 
cd learn2prog/18_reverse_str/
la
vi reverse.c
git add .
git commit -m "18"
git push
grade
git pull
cat grade.txt 
vi reverse.c
git add .
git commit -m "18"
git push
grade
cd learn2prog/19_bits_arr/
la
rm numToBits.c 
vi numToBits.v
vi numToBits.c
giit add .
git add .
git commit -m "19"
git push
grade
la
cd learn2prog/20_rot_matrix/
la
cd learn2prog/20_rot_matrix/
vi rotate.c
git add .
git commit -m "20"
git push
grade
git push
cd ../
git add .
c d20
cd 20_rot_matrix/
git add .
git commit -m "20"
git push
grade
ls
cd learn2prog/
ls
cd 21_read_rec1/
ls
vi test.c
vi answer.txt
git add .
git commit -m "21"
git push
grade
cd learn2prog/22_tests_power/
ls
cd /usr/local/l2p/power
ls
power.o 11 1
./power.0
./power.o
bash ./power.o
bash ./power.o 1 2
cd ~/learn2prog/22_tests_power
cd /usr/local/l2p/power
gcc power.o -o power
cd ~/learn2prog/22_tests_power
vi test-power.c
./run_all.sh 
bash /usr/local/l2p/power/power.o 
./run_all.sh /usr/local/l2p/power/power.o
./run_all.sh /usr/local/l2p/power/power0.o
cd learn2prog/21_read_rec1/
ls
cd ../
cd 22_tests_power/
ls
cd learn2prog/22
cd learn2prog/22_tests_power/
ls
vi a.c
¨er.c /usr/local/l2p/power/power.o
clear
ls
gcc -o a a.c /usr/local/l2p/power/power.o
vi a.c
gcc -o a a.c /usr/local/l2p/power/power.o
./a 
cd learn2prog/22_tests_power/
ls
vi test-power.c
rm test-power.c
vi test-power.c
./run_all.sh 
ll
rm test-power.c
vi test-power.c
./run_all.sh 
rm test-power.c
vi test-power.c
rm test-power.c
vi test-power.c
./run_all.sh 
rm test-power.c
./run_all.sh 
vi test-power.c
./run_all.sh 
git add .
git commit -m "22"
git push
grade
cd learn2prog/22_tests_power/
cd ../
git add .
git commit -m "22"
git push
cd 22_tests_power/
grade
cd learn2prog/23_power_rec/
ls
vi power.c
cd learn2prog/23_power_rec/
vi power.c 
rm power.c 
vi power.c 
git add .
git commit -m "23"
git push
grade
git pull
cat grade.txt 
vi power.c 
git add .
git commit -m "23"
git push
grade
git pull
cat grade.txt 
vi power.c 
git add /
git add .
git commit -m "23"
git push
grade
cd learn2prog/c3prj1_deck/
ls
vi deck.c
rm deck.c
vi deck.c
make 
./test-deck 
git add .
git commit -m "c3"
git push
grade
cd ../
git add .
git commit -m "c3"
git push
cd c3prj1_deck/
grade
git pull
cat grade.txt 
vi deck
vi deck.c
git add .
git commit -m "c3"
git push
grade
