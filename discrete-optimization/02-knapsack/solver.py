...
@njit
def go(path, value, room, estimate, estimate_all, item_count, picks, items):
    max_value = 0
...
def solve_it(input_data):
    ...
    items = np.zeros(item_count, dtype=[("value", np.int32), ("weight", np.int32)])
    ...
    for i in range(1, item_count + 1):
        line = lines[i]
        parts = line.split()
        items[i - 1]["value"] = int(parts[0])
        items[i - 1]["weight"] = int(parts[1])

    # 按价值密度从大到小排序
    value_density = items["value"] / items["weight"]
    sorted_indices = np.argsort(-value_density)  # 负号表示降序排列
    items = items[sorted_indices]

    left_room = capacity

    while left_room > 0:
        available_items = np.where((items["weight"] <= left_room) & (picks == 0))[0]
        if len(available_items) == 0:
            break

        value_densities = items[available_items]["value"] / items[available_items]["weight"]
        max_value_density_index = np.argmax(value_densities)
        max_value_density_item = available_items[max_value_density_index]

        if items[max_value_density_item]["weight"] <= left_room:
            left_room -= items[max_value_density_item]["weight"]
            picks[max_value_density_item] = 1
            max_estimate += items[max_value_density_item]["value"]
        else:
    ...
    result_path, result_value = go(np.array([0], dtype=np.int64), value, capacity, max_estimate, estimate_all, item_count, picks, items)

    value = result_value
    taken[sorted_indices] = result_path  # 恢复原始顺序
